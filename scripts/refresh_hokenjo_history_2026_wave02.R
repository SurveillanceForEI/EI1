...
source("R/hokenjo_fetch/ibaraki.R")
...
for (wk in 1:14) {
  fname <- if (wk == 1) "2026idwr1.pdf" else sprintf("2026idwr%02d.pdf", wk)
  u <- paste0("https://www.pref.ibaraki.jp/hokenfukushi/eiken/idwr/weekly/documents/", fname)
  res <- tryCatch(fetch_ibaraki(u), error = function(e) { status_log <<- c(status_log, sprintf("[NG] 第%d週: %s", wk, conditionMessage(e))); NULL })
