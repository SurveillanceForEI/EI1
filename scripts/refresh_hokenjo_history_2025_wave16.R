...
source("R/hokenjo_fetch/aichi.R")
...
for (week in 1:14) {
  res <- tryCatch(fetch_aichi(year = 2025, week = week), error = function(e) { status_log <<- c(status_log, sprintf("[NG] 愛知県 第%d週: %s", week, conditionMessage(e))); NULL })
  if (is.null(res) || nrow(res) == 0) next
  res$week_num <- week
  res$fetched_at <- as.character(Sys.time())
  new_rows[[length(new_rows) + 1]] <- res
  status_log <- c(status_log, sprintf("[OK] 愛知県 第%d週 (%d行) label=%s", week, nrow(res), unique(res$week_label)[1]))
}
