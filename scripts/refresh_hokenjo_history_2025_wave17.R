...
source("R/hokenjo_fetch/shimane.R")
...
for (week in 1:14) {
  u <- sprintf("https://pref.shimane.didss.dsvc.jp/files/report/week/weeklyreport_y2025w%02d.pdf", week)
  res <- tryCatch(fetch_shimane(u), error = function(e) { status_log <<- c(status_log, sprintf("[NG] 島根県 第%d週: %s", week, conditionMessage(e))); NULL })
  if (is.null(res) || nrow(res) == 0) next
  res$week_num <- week
  res$fetched_at <- as.character(Sys.time())
  new_rows[[length(new_rows) + 1]] <- res
  status_log <- c(status_log, sprintf("[OK] 島根県 第%d週 (%d行) label=%s", week, nrow(res), unique(res$week_label)[1]))
}
