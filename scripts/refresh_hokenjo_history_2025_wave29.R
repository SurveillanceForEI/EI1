...
# 高知県: 報告数0のセルが空欄(トークン無し)で表現されるのを誤ってNAに
# していたバグを修正(該当セルは0に)。以前雑にNA->0で一括置換していたが、
# 「報告数テーブルにその疾患の行自体が無い週」由来のNA（別原因、0とは
# 断定できない）まで巻き込んでいたため、正しいパーサーで全週を再取得し直す
before_n <- sum(h$pref == "高知県")
h <- h[h$pref != "高知県", ]
cat("高知県の既存行を削除:", before_n, "-> 0\n")
...
for (wk_str in names(kochi_urls)) {
  ...
  status_log <- c(status_log, sprintf("[OK] 第%d週 (%d行, countNA=%d, rateNA=%d)", week, nrow(res), sum(is.na(res$count)), sum(is.na(res$rate))))
}

cat("\n=== 実行ログ(2025年 第29弾:高知県 0/NA区別修正・再取得) ===\n")
