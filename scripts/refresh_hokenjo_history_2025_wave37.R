...
source("R/hokenjo_fetch/tochigi.R")
...
for (wk in 1:14) {
  u <- sprintf("https://www.pref.tochigi.lg.jp/e60/tidc/documents/intidwr2025%02d.pdf", wk)
  res <- tryCatch(fetch_tochigi(u), error = function(e) { status_log <<- c(status_log, sprintf("[NG] 第%d週: %s", wk, conditionMessage(e))); NULL })
