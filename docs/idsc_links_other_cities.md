...
| 藤沢市（神奈川県） | 藤沢市感染症発生状況（藤沢市保健所） | https://www.city.fujisawa.kanagawa.jp/kenko/iryo/kansensho/kansensho/hassei/index.html （ユーザー確認済み） | 候補なし（緊急情報RSS `https://www.city.fujisawa.kanagawa.jp/kinkyu/kinkyu.xml` は存在するが空で「緊急情報」専用のため新着/報道発表向けではない） | 低：センターURLはユーザー提供・確認済み。一般新着情報のRSS/JSON配信は見つからず、EBSニュース取得元としては引き続き未収録（継続調査中）。ページ自体は年度別発生状況（annual index、5年分）へのリンク集で、日付付きの逐次更新リストではない。 |
