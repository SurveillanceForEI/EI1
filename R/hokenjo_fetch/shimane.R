...
# 医療圏域: 松江,雲南,出雲,県央,浜田,益田,隠岐（保健所境界データの圏域名に対応）
# 各行は 地域(松江,雲南,出雲,県央,浜田,益田,隠岐,島根県) x 3週分の値がこの順で並ぶ。
# 「大田」は旧称。島根県サイト（nesid/weekly）のテーブルでは大田保健所は
# 「県央」に改称されている（列の並び順は松江,雲南,出雲の次のまま変わらず）。
# 松江保健所は松江市・島根県の共同設置で市域と県域で別々のcodeを持つが、
# 保健所としては1つなので地図上でも「松江」に統合する（2026-08-24 ユーザー指摘）。
...
fetch_shimane <- function(pdf_url) {
  ...
  regions <- c("松江", "雲南", "出雲", "県央", "浜田", "益田", "隠岐")
