...
# 注: who_eiosは含めない。WHO EIOSは世界中の一般メディア記事（日本語の国内ニュースを
# 大量に含む）を無差別に自動収集するアグリゲーターであり、台湾CDC等の「海外政府機関自身が
# 発信する自国の情報」とは性質が異なる。ここに含めると、国名等の地名キーワードを含まない
# 日本国内記事（例:「自民幹事長がインフル感染」「長野市で今季初のインフルエンザ集団感染」）
# まで一律「海外」と誤判定されてしまう（実例: 2026-09-14 EBS精度チェックで発見。
# 一般ロジック（!has_japan && has_overseas等）に委ねることで、明示的な海外キーワードが
# ある記事は引き続き海外と判定される）
