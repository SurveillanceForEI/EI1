...
# 疫学分野で頻出する語の中に、偶然そのまま国名や「グローバル」等の海外シグナル語が
# 埋め込まれてしまっているものを国名/海外判定マッチングの前に除去しておく
# （例:「サーベイランス」→「イラン」を誤検出する。「グローバルファンド」＝国際的な
# 保健基金の固有名詞だが、「グローバル」を含むため国内の外務省・厚労省による同基金
# 関連の会議出席報告等まで海外記事と誤判定されていた。2026-07-24 ユーザー報告で発覚）。
.COUNTRY_MATCH_FALSE_POSITIVE_WORDS <- c(
  "サーベイランス", "グローバルファンド",
  # WHO EIOS等がタイトル先頭に付与する記事カテゴリラベル。実際の記事内容と
  # 無関係に「国内」という語を含むため、日本国内記事と誤判定される
  # （実例:「国内・国際 ウガンダのエボラ熱が収束」2026-08-29 ユーザー指摘）
  "国内・国際"
)
...
is_noise_article <- function(title, summary = "") {
  ...
  # ── 5d. 無条件除外: 「コロナ禍/新型コロナの感染拡大」を過去の背景・原因として
  # 言及するのみで、主題が感染症と無関係な社会・教育・雇用・経営記事 ───────
  # 「…の感染拡大」という言い回しはhas_outbreak_core（次の6.）にも合致してしまい
  # 現在の流行文脈と誤認されるため、5cと同様にその判定より前に無条件除外する。
  # 例:「新型コロナウイルスの感染拡大当時を振り返り…在宅勤務を完全廃止」
  # （実例: 2026-07-24 ユーザー指摘）
  if (grepl("コロナ.{0,10}(禍|の影響|感染拡大)", tl, perl = TRUE) &&
      grepl(paste0("学生募集|入学者(数)?(減少|激減)|存続困難|",
                   "在宅勤務|テレワーク|リモートワーク|",
                   "人手不足|人材(育成|不足)|採用(強化|活動)|",
                   "業績|売上|経営(不振|改善)|インバウンド|観光客"),
            tl, perl = TRUE)) return(TRUE)

  # 5d-2. 無条件除外: 「新型コロナの感染拡大で外出ができなくなった時期に◯◯を
  # 始めた/再開した」のような、個人の趣味・生活習慣の変化を語る人間模様記事
  # （コロナ禍を単なる時代背景として言及するのみで、現在の流行報告ではない）
  # 例:「新型コロナウイルスの感染拡大で外出ができなくなった時期にピアノを
  # 再開した」トチリコンサート記事（2026-08-29 ユーザー指摘）
  if (grepl("(感染拡大|コロナ禍).{0,15}(外出|自粛|巣ごもり|ステイホーム)", tl, perl = TRUE) &&
      grepl("きっかけ|再開|始めた|再び|趣味", tl, perl = TRUE)) return(TRUE)

  # 5d-3. 無条件除外: 「感染拡大前の水準」のように、コロナ禍前後を単なる
  # 時期の基準点として用いる比較表現（観光・交通量等の回復状況の話題で、
  # 現在の流行報告ではない）。has_outbreak_coreは「感染拡大」という語句
  # だけで合致してしまうため、その判定より前に無条件除外する
  # 例:「台湾の国内線も感染拡大前の水準に近づいている」（2026-08-29 ユーザー指摘）
  if (grepl("感染拡大(前|後)の?水準|感染拡大(前|後).{0,10}(回復|近づ)",
            tl, perl = TRUE)) return(TRUE)

  # ── 5e. 無条件除外: 過去の集団発生の追悼・回顧記念記事 ────────────────
  # 「堺市集団食中毒30年で追悼　9千人以上がO157感染」のように、大規模な
  # 症例数への言及があるため通常はhas_outbreak_core（次の6.）に合致して
  # しまうが、実際には数十年前の出来事の記念・追悼報道であり現在の流行では
  # ない。5c/5dと同様、その判定より前に無条件除外する
  # （実例: 2026-07-24 ユーザー指摘）
  if (grepl(paste0(
    "[0-9０-９]{1,3}年.{0,10}(で追悼|目.{0,5}(追悼|慰霊)|風化)|",
    "(発生|感染)から[0-9０-９]{1,3}年"
  ), tl, perl = TRUE)) return(TRUE)

  # ── 5f. 無条件除外: ワクチン・診断薬・治療法の研究開発・資金提供ニュース ──
  # 「GHIT Fund、エボラウイルス病の迅速診断システムの開発に1億4000万円を投資」
  # のように、感染症名（一類感染症等special_pathogenに合致するものを含む）が
  # 出てきても、主題は研究開発・助成の話であり現在の発生状況の報告ではない。
  # special_pathogen等の重大疾患判定より前に無条件除外する必要があるため
  # ここに置く（実例: 2026-07-24 ユーザー指摘）
  if (grepl(paste0(
    "(診断システム|診断キット|診断法|検査キット|治療薬|治療法|ワクチン).{0,15}",
    "(の)?開発.{0,15}(着手|投資|助成|成功|完了)|",
    "(投資|助成|基金|fund|grant).{0,15}開発|",
    "開発.{0,10}(に)?(着手|投資|助成)"
  ), tl, perl = TRUE)) return(TRUE)

  # ── 5g. 無条件除外: 施設立地・行政制度に関する論説・意見記事 ──────────
  # 「なぜBSL-4施設を東京に置きたがるのか：「顔の見える関係」という時代錯誤」
  # のような、感染症研究施設の立地等をめぐる論説・意見記事
  if (grepl("なぜ.{0,20}(置きたがる|たがるのか)|時代錯誤", tl, perl = TRUE)) return(TRUE)

  # ── 5h. 無条件除外: 行政計画・パブリックコメント関連文書 ─────────────
  # 「目黒区新型インフルエンザ等対策行動計画（改定素案）に対するパブリック
  # コメントの実施結果」のような、感染症対策の行動計画そのもの・意見募集の
  # 告知や結果報告。現在の発生状況の報告ではない
  if (grepl("行動計画|パブリックコメント|意見募集|改定素案", tl, perl = TRUE)) return(TRUE)

  # ── 5i. 無条件除外: 「〜をなくすには」型の一般啓発記事 ───────────────
  # 「先天性風疹症候群をなくすには」のような、疾患の撲滅・予防を訴える
  # 一般向け啓発記事（現在の発生状況の報告ではない）
  if (grepl("をなくすには|なくす.{0,5}(には|ために)", tl, perl = TRUE)) return(TRUE)

  # ── 5j. 無条件除外: 戦争・紛争の「勃発」を指すoutbreak ───────────────
  # 「outbreak of the Iran war」のように、warやconflict等と結び付いた
  # outbreakは戦争の勃発を指す一般語義であり、感染症のアウトブレイクでは
  # ない。has_outbreak_core（次の6.）より先に無条件除外する必要がある
  # （実例: Japan Times「power futures」記事、2026-09-11チェックで発見）
  if (grepl(paste0(
    "outbreak of.{0,20}(war|conflict|hostilities|fighting)|",
    "(war|conflict).{0,20}outbreak|戦争.{0,5}勃発|紛争.{0,5}勃発"
  ), tl, perl = TRUE)) return(TRUE)
  ...
  # 17b. 個人の罹患体験談・エッセイ（芸能人に限らず、流行状況の報告ではなく
  # 個人の闘病記・思い出語り等）。例:「〜はインフルエンザ、COVID-19にり患し…」。
  # 症例数等の具体的な流行報告（has_outbreak_core）には既に合致しない
  # ことが確認済みのため、ここでは「個人のり患」＋「体験談的な文脈」の
  # 組み合わせのみを見る（2026-07-24 ユーザー指摘）。
  if (grepl(paste0("(り患|罹患|感染).{0,15}(し|した|して).{0,40}",
                   "(闘病|振り返|語った|語る|綴った|つづった|エッセイ|自伝|",
                   "告白|思い出|経験を|エピソード|休養|休演|欠席した)"),
            tl, perl = TRUE)) return(TRUE)

  # 17c. プロ野球選手個人の調子・状態を語る記事で、感染症（インフルエンザ等）は
  # 過去に罹患したことがコンディション低下の一因として触れられるのみで、
  # 主題は選手個人のファーム・調整状況（流行報告ではない）。
  # 例:「【ロッテ藤岡裕大】苦難の現状でもファームで貫く流…」（2026-08-24 ユーザー指摘）
  if (grepl("ファーム|２軍|2軍", tl, perl = TRUE) &&
      grepl("インフル|感染|コロナ", tl, perl = TRUE) &&
      grepl("状態|調子|きっかけ", tl, perl = TRUE)) return(TRUE)

  # 18. 食中毒予防・衛生管理の一般啓発（集団発症報告でない）。ただし「食中毒
  # 警報」等で具体的な発生件数・有症者数が示されている場合は実際の集団発症
  # データであり除外しない（例:「岐阜県と岐阜市 食中毒警報…有症者は２４５人」
  # が誤って除外されていた事例、2026-08-24 関連スキャンで発見）
  if (grepl(paste0("食中毒.*(予防|気をつける|対策|注意.*[夏季]|衛生管理)|",
                   "衛生管理.*細心.*注意|弁当.*加熱.*菌"),
            tl, perl = TRUE) &&
      !grepl("(有症者|発症者|患者).{0,10}[0-9０-９]+人|[0-9０-９]+人.{0,10}(発症|感染)",
             tl, perl = TRUE)) return(TRUE)
  ...
  # 「Ｂ型肝炎」は全角Ｂのため小文字化しても半角bにならず、以前は
  # 「b型肝炎.*訴訟」パターンにマッチしなかった（2026-08-24 関連スキャンで発見）
  if (grepl(paste0("薬害エイズ.*(救済|充実|原告|厚労相)|",
                   "薬害.*原告団.*厚労|[bBＢｂ]型肝炎.*訴訟"),
            tl, perl = TRUE)) return(TRUE)
  ...
  # 54. 脱毛症・美容医療（感染症は脱毛原因の一例として言及されるのみ）
  # 例:「脱毛症、再生医療で新たな治療法…毛のある後頭部から細胞を採取…」
  # （2026-08-24 ユーザー指摘・関連スキャンで発見）
  if (grepl("脱毛症|薄毛|抜け毛", tl, perl = TRUE) &&
      grepl("再生医療|治療法|お肌のトラブル|美容", tl, perl = TRUE)) return(TRUE)

  # 55. 検査キット市場・企業決算（感染症流行そのものでなく企業業績の報道）
  # 例:「株式会社タウンズ：2026年6月期決算説明会文字起こし」
  # （2026-08-24 関連スキャンで発見）
  if (grepl("決算|決算説明会|営業利益|売上高", tl, perl = TRUE) &&
      grepl("検査キット|前期比", tl, perl = TRUE)) return(TRUE)

  # 56. 自治体の電子申請・手続き案内ページ（疾患名は接種券再発行等の
  # 事務手続き文脈で言及されるのみで、流行状況の報告ではない）
  # 例:「久留米市：電子申請」（2026-08-24 関連スキャンで発見）
  if (grepl("電子申請", tl, perl = TRUE) &&
      grepl("利用規約|システム|申請画面", tl, perl = TRUE)) return(TRUE)

  # 57. 有名人（タレント・アイドル・俳優・歌手）本人の個人罹患・入院報告
  # （闘病を振り返るトーク番組出演や、活動休止のお知らせ等）。17/17bの
  # パターンでは語順が一致せず漏れていた事例を補完する。
  # 例:「JOY『よく生きてましたねって状態だった』…肺結核になった」
  # 「大分発アイドル、入院を報告…マイコプラズマ肺炎と診断」（2026-08-24 関連スキャンで発見）
  if (grepl("タレント|アイドル|俳優|歌手", tl, perl = TRUE) &&
      grepl("結核|肺炎|マイコプラズマ|インフルエンザ", tl, perl = TRUE) &&
      grepl("入院|休演|治療に専念|語った|振り返|報告", tl, perl = TRUE)) return(TRUE)

  # 58. テレビドラマの制作発表・キャスト発表（劇中設定としての疾患言及で
  # あり、現実の流行報告ではない）
  # 例:「NHK主演ドラマ…口蹄疫も扱う」「日本テレビ系水曜ドラマ…先天性心疾患
  # を抱えた特定妊婦役」（2026-08-24 関連スキャンで発見）
  if (grepl("ドラマ", tl, perl = TRUE) &&
      grepl("制作発表|会見|ゲスト発表|キャスト", tl, perl = TRUE) &&
      grepl("感染症|口蹄疫|心疾患|結核|コロナ", tl, perl = TRUE)) return(TRUE)

  # 59. 「偽ウイルス感染」等を騙るフィッシング詐欺の注意喚起（実際の感染症
  # ではない。「ウィルス」表記はcyber_fixedの半角イ表記と揺れがあるため別途）
  # 例:「消費にゅーす『偽ウィルス感染詐欺の手口』について」（2026-08-24 関連スキャンで発見）
  if (grepl("(偽|なりすまし).{0,10}(ウイルス|ウィルス).{0,10}感染", tl, perl = TRUE) &&
      grepl("詐欺|フィッシング", tl, perl = TRUE)) return(TRUE)

  # 60. 太平洋戦争の戦時中の飢餓・マラリア等への言及を含む慰霊・戦争体験
  # 回顧コラム（現在の流行と無関係な歴史・平和テーマ）
  # 例:「＜金口木舌＞海の戦争…住民が飢えとマラリアに苦しんだ…慰霊祭」
  # （2026-08-24 関連スキャンで発見）
  if (grepl("沖縄戦|地上戦|艦砲射撃|疎開", tl, perl = TRUE) &&
      grepl("マラリア|飢え", tl, perl = TRUE) &&
      grepl("慰霊|継承|遺族|平和", tl, perl = TRUE)) return(TRUE)

  # 61. 「今日は何の日」的な歴史カレンダートリビア記事（感染症の歴史上の
  # できごとが列挙されるのみで、現在の流行報告ではない）
  # 例:「今日はどんな日. 蚊の日. ◆出来事…ロスがマラリアの原虫を発見（1897）」
  # （2026-08-24 関連スキャンで発見）
  if (grepl("今日はどんな日|◆出来事", tl, perl = TRUE) &&
      grepl("マラリア|インフルエンザ|コレラ|結核|ペスト", tl, perl = TRUE)) return(TRUE)

  # 62. 訃報・追悼記事（死因が感染症であることの言及がないもの）。俳人・
  # 文化人・著名人の死去記事の経歴紹介中に「結核予防」等の過去の功績が
  # 触れられるだけで、現在の流行報告ではないケースを想定
  # 例:「木割大雄さん死去…下町のプロデューサー87歳」
  # 「香淳皇后…結核予防に尽力」（2026-08-29 ユーザー指摘）
  if (grepl("死去|逝去|死没|享年|訃報", tl, perl = TRUE) &&
      !grepl("(死者|死亡者)[0-9０-９]+\\s*(人|名|例)|感染.{0,10}(死去|死亡)",
             tl, perl = TRUE)) return(TRUE)

  # 63. 皇室・皇族の経歴・公務紹介記事（過去の公衆衛生活動への言及があっても
  # 現在の流行報告ではない）
  if (grepl("皇族|皇后|天皇|上皇|皇太子", tl, perl = TRUE) &&
      grepl("公務|生涯|人生|激動の時代|覚悟", tl, perl = TRUE)) return(TRUE)

  # 62b. 病原体・病因の基礎生命科学研究（酵素・分子メカニズムの発見等で、
  # 現在の流行・症例報告ではない）
  # 例:「病原体生体膜リン脂質合成酵素(LPLAT)群の発見～新たな創薬ターゲット～」
  # （2026-08-29 ユーザー指摘）
  if (grepl("創薬ターゲット|酵素.{0,10}(発見|同定)|分子メカニズム.{0,10}(解明|発見)",
            tl, perl = TRUE) &&
      !grepl("outbreak|アウトブレイク|感染者|患者[0-9]", tl, perl = TRUE)) return(TRUE)

  # 64. コンサート・カーニバル・祭り等のイベント告知・開催報告（感染症流行と
  # 無関係な文化イベント）
  # 例:「浅草サンバカーニバル」「トチリコンサート」（2026-08-29 ユーザー指摘）
  # （has_outbreak_coreが既にFALSEであることが確定しているため、現在進行形の
  # 流行を理由とした開催是非の記事はこの時点には到達しない。「コロナ禍で
  # 中止された年もあった」等の過去の振り返り言及は除外条件にしない）
  if (grepl("コンサート|カーニバル|フェスティバル|花火大会|盆踊り", tl, perl = TRUE)) return(TRUE)

  # 65. 手術支援ロボット等の医療技術ニュース（感染症流行と無関係な医療機器・
  # 術式の紹介）
  # 例:「ロボ支援で高度心臓手術」（2026-08-29 ユーザー指摘）
  if (grepl("ロボ(ット)?支援|支援ロボット", tl, perl = TRUE) &&
      grepl("手術", tl, perl = TRUE)) return(TRUE)

  # 66. 戦没者追悼式・慰霊祭（感染症流行と無関係な戦争関連の式典）
  # 例:「凄惨な記憶継承　水戸で県戦没者追悼式」（2026-08-29 ユーザー指摘）
  if (grepl("戦没者.{0,10}(追悼式|慰霊祭|式典)|追悼式.{0,10}戦没者", tl, perl = TRUE)) return(TRUE)

  # 66c. 政治家・首長のパワハラ・不祥事報道（経歴紹介中に「コロナ専門家」等の
  # 過去の肩書が触れられるのみで、現在の流行報告ではない）
  # 例:「パワハラ認定で主要会派が見切り、道半ばで退場…横浜」（2026-08-29 ユーザー指摘）
  if (grepl("パワハラ", tl, perl = TRUE) &&
      grepl("市長|議会|会派|議員|退場|見切り", tl, perl = TRUE)) return(TRUE)

  # 66b. 国際政治・戦争・選挙報道（感染症流行と無関係）
  # 例:「Republicans see Trump's unceasing Iran war as midterm threat」
  # （2026-08-29 ユーザー指摘）
  if (grepl(paste0("midterm|republicans?|democrats?|",
                   "(iran|israel|russia|ukraine|gaza).{0,20}war|",
                   "war.{0,20}(iran|israel|russia|ukraine|gaza)|",
                   "election campaign|presidential race"),
            tl, perl = TRUE) &&
      !grepl("outbreak|epidemic|感染|infectious disease|disease outbreak",
             tl, perl = TRUE)) return(TRUE)

  # 67. 航空便の運航スケジュール変更（検疫・防疫目的の運航停止でない限り、
  # 感染症流行と無関係な路線・便数の増減ニュース）
  # 例:「マンダリン航空が運航停止申請の花蓮路線…減便」（2026-08-29 ユーザー指摘）
  if (grepl("(運航停止|減便|路線).{0,15}(航空|便)|(航空|便).{0,15}(運航停止|減便)",
            tl, perl = TRUE) &&
      !grepl("検疫|隔離", tl, perl = TRUE)) return(TRUE)

  # 68. 芸能人・スポーツ選手個人の体調不良報道（公衆衛生上のサーベイランス対象
  # ではない、個人の闘病・仕事復帰等の芸能ニュース）
  # 例:「高橋文哉、インフルエンザによる体調不良から仕事復帰」
  # 「和田アキ子 体調不良疑い病院へ」（2026-09-11 EBS精度チェックで発見）
  if (grepl(paste0(
    "(体調不良|療養).{0,10}(から)?(仕事復帰|活動再開|舞台復帰)|",
    "体調不良疑い.{0,10}病院|くそっ"
  ), tl, perl = TRUE)) return(TRUE)

  # 69. 生活情報・レシピ記事（免疫力アップ等をうたう食材紹介で、実際の感染症
  # 発生報告ではない）
  # 例:「ウイルスに勝て！免疫力アップは緑黄色野菜＆果物から」
  # 「風邪予防にフルーツの力を借りてみては？」（2026-09-11 EBS精度チェックで発見）
  if (grepl(paste0(
    "免疫力アップ|ウイルスに勝て|風邪予防に.{0,10}(の)?力を借り|",
    "野菜.{0,10}果物.{0,10}(から|で)|吹き飛ばせ"
  ), tl, perl = TRUE)) return(TRUE)

  # 70. 通信販売・キャンペーン等の広告記事（ワクチン接種や検査キットの実施
  # 状況ではなく、商品・サービスの宣伝が主目的）
  # 例:「自己検査キットの準備とオンライン診療の活用を。【クーポンコードあり】」
  # （2026-09-11 EBS精度チェックで発見）
  if (grepl("クーポンコード|【PR】|プロモーション対象商品", tl, perl = TRUE)) return(TRUE)

  # 71b. AI（生成AI）によるウイルス設計・デザイン研究報道（実際の感染症
  # 発生・流行ではなく、計算生物学・バイオセキュリティ分野の研究ニュース）
  # 例:「生成AIが「新ウイルス」を設計 米スタンフォード大、増殖を確認」
  # 「AIは「ディープフェイク・ウイルス」を作れるようになった」
  # （2026-09-14 EBS精度チェックで発見）
  if (grepl("(生成)?ai.{0,10}(ウイルス|virus).{0,10}(設計|生成|デザイン)|ディープフェイク・ウイルス",
            tl, perl = TRUE)) return(TRUE)

  # 71. 出版・グッズ等の商品紹介記事（疾患名を含む書籍・図鑑・キャラクター
  # 商品の発売告知で、実際の感染症発生報告ではない）
  # 例:「大人気『はたらく細胞』図鑑シリーズ最新刊がついに発売！」
  # （2026-09-11 EBS精度チェックで発見）
  if (grepl("図鑑シリーズ|イラスト満載|最新刊.{0,10}(が)?ついに発売", tl, perl = TRUE)) return(TRUE)
