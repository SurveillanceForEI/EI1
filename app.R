...
$(document).on("shown.bs.tab", "a[data-toggle=\'tab\']", function() {
  ...
  setTimeout(function() {
    ...
    tabBox(width=12, id="main_tabs",
      ...
      tabPanel("保健所別比較", icon=icon("map-location-dot"),
        tags$div(style="padding:10px 6px 4px;",
          tags$p(style="color:#666;font-size:0.85em;",
            ...
            tags$br(),
            "※ 保健所別データは各自治体が公表している週報等をもとに収集しているため、",
          ...
          tags$h5("週別集計表（データ取得済み期間〜選択週、保健所別）", style="font-weight:700;margin-top:10px;"),
          tags$style(HTML("
          ...
          tags$ul(
            ...
            tags$li(tags$strong("その他:"),
              tags$ul(
                ...
                tags$li(tags$strong("EIとは:"), "IBS・EBSを用いたEpidemic Intelligence活動についての解説"),
      ...
      # ── Epidemic Intelligence活動について ─────────────────
      tabPanel("EIとは", icon=icon("chalkboard-user"),
        tags$div(style="padding:20px;max-width:960px;font-size:0.9em;line-height:1.8;",
          ...
          tags$p(
            ...
            "大きく「IBS」と「EBS」の2つに整理され、",
            "それぞれの対応（アウトプット）につながります。双方の結果は継続的なリスク評価に集約され、",
            "自治体等における具体的な対応判断を支えます。"
          ),

          tags$div(style="text-align:center;margin:14px 0;",
            tags$img(src="ei_overview_diagram.png", alt="サーベイランスとEIの全体像",
          ...
          tags$p(style="font-size:0.85em;color:#888;margin-top:10px;",
            "本ダッシュボードでは、「患者サーベイランス」「病原体サーベイランス」「入院サーベイランス」等の各タブがIBSに、",
            "「EBS」タブがEBSに対応しています。"),
          tags$br(),

          # ── ② プロセス ────────────────────────────────────
          tags$h4("② EI活動のプロセス", style="border-bottom:2px solid #4b499c;padding-bottom:4px;color:#2c3e50;"),
          tags$p("IBSデータ・EBS情報は、以下のプロセスを経て評価・対応につながります。"),
          ...
          tags$div(style="display:flex;align-items:stretch;gap:6px;flex-wrap:wrap;justify-content:center;margin:14px 0;",
            lapply(list(
              ...
              tagList(
                ...
                tags$div(style="align-self:center;color:#8b8bda;font-size:1.1em;", icon("arrow-right"))
              )
            })
          ),
          tags$p(style="font-size:0.85em;color:#888;margin-top:10px;",
          ...
          # ── ③ 検証〜さらなるアクション ─────────────────────
          tags$h4("③「検証」から「分析・リスク評価」「アクション」へ", style="border-bottom:2px solid #4b499c;padding-bottom:4px;color:#2c3e50;"),
          tags$h5("検証"),
          tags$ul(
            tags$li("シグナルやイベントと判断した情報の内容を確認し、それぞれのフィルタリングが妥当か関係者内で確認する"),
            tags$li("この過程は不足している情報等を洗い出す段階であり、フィルタリングの理由は、その時判断した担当者が説明できれば問題ない"),
            tags$li("積極的モニタリング・分析・リスク評価等につなげる必要があるかを、関係者で連携しながら検証する")
          ),
          tags$h5("分析・リスク評価"),
          tags$ul(
            tags$li("リスク評価の際には、複数のデータ・情報を活用して分析・評価を行う"),
            tags$li("この段階では非公開情報も多く含むため、その結果は関係者内での共有にとどまることも多い")
          ),
          tags$h5("アクション"),
          tags$ul(
            tags$li("評価結果を踏まえ、さらなる積極的な情報収集を検討する"),
            tags$li("関係機関への情報提供や外部機関からの情報収集、積極的疫学調査を検討する")
          )
        )
      ),
...
server <- function(input, output, session) {
  ...
  hokenjo_summary_table_data <- reactive({
    ...
    wide <- tidyr::pivot_wider(sub[, c("hokenjo", "week_col", metric)],
                                names_from = "week_col", values_from = dplyr::all_of(metric))
    wide <- wide[, c("hokenjo", col_levels)]
    names(wide)[names(wide) == "hokenjo"] <- "保健所"
    wide
  })
